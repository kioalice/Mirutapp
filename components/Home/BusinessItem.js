...
function BusinessItem({business,showDir=false}) {
    ...
    const calculateDistance = (lat1, lon1, lat2, lon2) => {
        ...
        const distancee = earthRadius * c;
        const distance = distancee * (1609)
  ...
  return (
    <div className='w-[195px] flex-shrink-0 p-2 rounded-lg shadow-md mb-1 bg-white  hover:scale-110 transition-all mt-[20px] cursor-pointer'>
        ...
        <h2 className='text-[13px] font-bold mt-1 line-clamp-1'>{business.name}</h2>
              ...
              flex justify-between items-center'>Dist: {distance} Km Aprox. 
              <span className='border-[1px] p-1 rounded-full
              border-blue-500
              hover:text-white
              hover:bg-blue-500 text-center' onClick={()=>onDirectionClick()} >Ir a la dirección</span></h2>
            </div>:null}
