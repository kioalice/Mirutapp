...
function BusinessItem({business, showDir=false}) {
    ...
    const calculateDistance = (lat1, lon1, lat2, lon2) => {
        ...
        setDistance(distanceInKilometers.toFixed(1)); // Set distance state with 1 decimal place
        return distanceInKilometers.toFixed(2); // Return the distance with 2 decimal places
    };

    const onDirectionClick = () => {
        window.open(`https://www.google.com/maps/dir/?api=1&origin=${userLocation.lat},${userLocation.lng}&destination=${business.geometry.location.lat},${business.geometry.location.lng}&travelmode=driving`);
    }
            ...
            {showDir &&
                <div className='border-t-[1px] p-1 mt-1'>
                    <h2 className='text-[#0075ff] flex justify-between items-center'>
                        Dist: {distance} Km Aprox. 
                        <span className='border-[1px] p-1 rounded-full border-blue-500 hover:text-white hover:bg-blue-500 text-center' onClick={onDirectionClick}>
                            Ir a la dirección
                        </span>
                    </h2>
                </div>
